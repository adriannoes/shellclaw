...
This document covers marketplace registration for ShellClaw v1.0. For manifest publishing, signing, and key rotation, see the repository root `AGENTS.md` and `.cursor/dev-planning/tasks/phase5/03-inference-trust-marketplace.md`.
...
Manifest code registers skills from `skills/*.md` on disk; default descriptions in `src/asap/manifest.c` align with the four IDs above.

...
static size_t write_cb(char *ptr, size_t size, size_t nmemb, void *userdata)
{
	size_t n = size * nmemb;
	(void)userdata;
	if (g_response_buf == NULL) return 0;
	size_t need = g_response_len + n + 1;
	if (need > RESPONSE_BUF_INIT * 4) return 0;
	char *p = realloc(g_response_buf, need);
	if (!p) return 0;
	g_response_buf = p;
	memcpy(g_response_buf + g_response_len, ptr, n);
	g_response_len += n;
...
static int parse_response_body(provider_response_t *response)
{
...
static int do_request(const char *body, provider_response_t *response)
{
	...
	g_response_buf = malloc(RESPONSE_BUF_INIT);
	g_response_len = 0;
	if (!g_response_buf) {
	...
	char key_header[128];
	snprintf(key_header, sizeof(key_header), "x-api-key: %s", g_api_key);
	...
	int ret = parse_response_body(response);
	free(g_response_buf);
	g_response_buf = NULL;
	return ret;
}
